...
This page is meant for people trying to get into speedrunning [**Dark Souls**](/darksouls) Prepare to Die Edition (PTDE).
...
As is true for learning any Souls title, it is highly recommended to join the [**SoulsSpeedruns Discord server**](https://discord.soulsspeedruns.com/). There are various channels dedicated to each game, including help channels, with knowledgeable people around that can help with just about any question.
...
<YouTube id="https://www.youtube.com/watch?v=6L5czTfolMk?list=PL94r3kPzSJlhSZTiHfwk3Z31D-iQiLNAE" />

## Choosing a Category & Route

You can refer to the main categories [**here**](/darksouls#leaderboards--categories) and make your choice. You often have the option between multiple routes. You may choose to immediately go for the fastest (and usually world record) route, but these can require a high execution level. If you are willing to jump right into one of them, feel free to do so. If you wish to take it easier first, there is good beginner alternatives you can start out with and still achieve great times.

Here follows a list of current options for the categories a beginner might choose:

##### **Any%**

- BKGS with Ladderwarp ([Record by QueueKyoo](https://youtu.be/9jp9_uUJSt0))
- BKGS +5 Beginner route ([Video Tutorial by Regole](https://youtu.be/ZEw_idEKImA) / [Route Notes](https://pastebin.com/z60WfZHz))

##### **No Wrong Warp**

- BKH with Ladderwarp ([Record by xmonkeh](https://youtu.be/XemhL98nn-M))
- BKH without Ladderwarp ([Run by xmonkeh](https://youtu.be/WaL6HS3nv1Q) / [Route Notes](/darksouls/black-knight-halberd-any-no-wrong-warp))
  - Without Crystal Caves Slope Quit ([Run by Moochyy_](https://youtu.be/Y3yVK-_5Jc0))
  - Without Blighttown Skip & Crystal Caves Slope Quit ([Run by commanderAIK](https://www.twitch.tv/videos/1076111454))

##### **All Bosses**

- Dragon Tooth +5 (Route name: Bluetooth) ([Record by xmonkeh](https://youtu.be/Mw6KMnKO_EI))
- Dragon Tooth + Sorcery Beginner Route (Route name: ESMailless) ([Video by Regole](https://youtu.be/zD-wplsOlWE) / [Route Notes](/darksouls/esmailless))

For routes requiring a random drop, such as the Black Knight Greatsword and Black Knight Halberd, there is a tool called [Gadgetlemage](https://github.com/CapitaineToinon/Gadgetlemage/releases). Download the .zip, extract the .exe, run it and set it to whatever weapon it is that you are going for. Whenever the weapon does not drop after killing the Black Knight, the program will automatically insert it into your inventory. This makes it a great way to run and practice the route without needing to get a weapon drop, while not altering the game if you do get a drop by the game. (Runs without a drop are not valid for leaderboards at the present time.)
...
- The game's [**leaderboards**](https://www.speedrun.com/darksouls) on speedrun.com.
- A **Keyboard** or **Controller**. Most Dark Souls speedrunners prefer controller, commonly Xbox or PlayStation controllers. Controllers in Dark Souls make the game more friendly to play, but it is all up to preference in the end. Keep in mind that at high-level play, a mix of both your controller and keyboard may be favourable. Controller rebinds are commonly used, especially for the [Prompt Swap](/darksouls/prompt-swap) glitch. See the playlist by AtiTheHedgehog above for more info.
- If you decide to use the **Keyboard and Mouse** combination, it is strongly recommended to use the [**DS Mouse Fix**](https://nexusmods.com/darksouls/mods/28) as manipulating the mouse without it can be unnecessarily annoying.
...
- [**Gadgetlemage**](https://github.com/CapitaineToinon/Gadgetlemage/releases). Not to be confused with Gadget. When this program is open, it automatically inserts Black Knight weapons into your inventory whenever you kill a Black Knight and do not get weapon drop.
